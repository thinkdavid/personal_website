...
test('collectPhotoPaths throws when a marker folder has no images', async () => {
  ...
  await assert.rejects(
    ...
    /expected peru\/landscape and peru\/portrait/i,
  );
});
...
test('publishWorkEntry writes backups, work page, and updated index/gallery via injected dependencies', async () => {
  ...
  const writtenByPath = Object.fromEntries(
    writes.map((write) => [write.path, write.contents]),
  );
  assert.match(
    writtenByPath['work/iceland-dawn.html'],
    /work-template:iceland-dawn/,
  );
...
test('publishWorkEntry detects duplicate slug links with .html in relative and absolute forms', async () => {
  ...
  for (const href of duplicateHrefs) {
    ...
    const loadDeps = async () => ({
      readTextFile: async (_root, path) => {
        if (path === 'workSnippetWithVariables.html') return 'snippet-template';
        if (path === 'work/work-page-with-variables.html')
...
test('buildWorkPageHtml escapes title and subtitle in text and alt attributes', () => {
  ...
  const escapedSubtitle =
    '&lt;script&gt;alert(&quot;x&quot;)&lt;/script&gt; &amp; rocks';
...
test('buildWorkPageHtml omits cover photo from landscape gallery items', () => {
  const html = buildWorkPageHtml(WORK_TEMPLATE_WITH_DESCRIPTION, {
    ...
    landscapePhotos: [
      'peru/landscape/DSC_0818.jpg',
      'peru/landscape/DSC_0845.jpg',
    ],
    portraitPhotos: [],
  });
  ...
  assert.equal(
    html.includes(
      '<img loading="lazy" height="Auto" alt="" src="../peru/landscape/DSC_0818.jpg" sizes="(max-width: 479px) 92vw, 95vw" srcset="../peru/landscape/DSC_0818.jpg" class="work-image">',
    ),
    false,
  );
});
...
test('buildSnippetHtml sanitizes malicious cover photo path for html attributes and srcset', () => {
  const maliciousPath =
    'iceland/cover" onerror="alert(1)</script><script>alert(2)</script>.jpg';
  const template = `
    <a href="/work/{urlSlug}">
...
test('buildWorkPageHtml sanitizes malicious cover and gallery paths for attributes, srcset, and lightbox json', () => {
  ...
  const portraitPath =
    'iceland/portrait/pose"</script><script>alert(1)</script>.jpg';
  ...
  assert.match(
    html,
    /src="\.\.\/iceland\/cover%22%20onerror%3D%22alert\(1\)\.jpg"/,
  );
  assert.match(
    html,
    /src="\.\.\/iceland\/landscape\/frame%22%20onclick%3D%22evil\(1\)\.jpg"/,
  );
  assert.match(
    html,
    /src="\.\.\/iceland\/portrait\/pose%22%3C\/script%3E%3Cscript%3Ealert\(1\)%3C\/script%3E\.jpg"/,
  );
  assert.match(
    html,
    /"url":"\.\.\/iceland\/portrait\/pose%22%3C\/script%3E%3Cscript%3Ealert\(1\)%3C\/script%3E\.jpg"/,
  );
