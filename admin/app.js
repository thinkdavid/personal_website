...
function validateForm() {
  ...
  if (data.workNameInput && !data.workName) {
    setFieldError(
      'workName',
      'Work folder name is invalid after sanitizing. Use letters and numbers.',
    );
    valid = false;
  }
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.warn(
          'Blob Storage not available, falling back to repository:',
          error.message,
        );
        setStatus(
          'Warning: Blob Storage unavailable, using repository upload',
          'warn',
        );
      }
    }

    const { landscapePhotos, portraitPhotos } = await collectPhotoPaths(
      state.rootHandle,
      data.workName,
    );
    const payload = {
    ...
    const { snippetHtml, workPageHtml } = await publishWorkEntry(
      state.rootHandle,
      payload,
      containerClient,
    );
    ...
    setStatus(
      `Published "${data.slugValue}" with images uploaded to ${uploadMethod}.`,
      'success',
    );
  } catch (error) {
...
async function selectRoot() {
  try {
    ...
    setStatus(
      'Project folder selected. Generate and publish when ready.',
      'success',
    );
  } catch (error) {
...
function wireEvents() {
  ...
  el.slug.addEventListener('input', () => {
    state.slugAuto =
      el.slug.value.trim() === '' ||
      el.slug.value.trim() === slugify(el.title.value);
  });
}
